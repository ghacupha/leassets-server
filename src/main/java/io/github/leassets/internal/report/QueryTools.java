package io.github.leassets.internal.report;
...
public class QueryTools {
    ...
    public static String toSQL(Criteria crit) {
        String sql = new BasicFormatterImpl()
            .format(
                (
                    new CriteriaJoinWalker(
                        (OuterJoinLoadable) ((CriteriaImpl) crit).getSession()
                            .getFactory()
                            .getEntityPersister(
                                ((CriteriaImpl) crit).getSession()
                                    .getFactory()
                                    .getImplementors(((CriteriaImpl) crit).getEntityOrClassName())[0]
                            ),
